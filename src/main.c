            ...
            switch (event.type) {
                ...
                case SDL_EVENT_KEY_DOWN:
                    ...
                    if (event.key.key == SDLK_TAB) {
                        ...
                        loop_target_distance_m = openride_app_support_clampd(
                ...
                case SDL_EVENT_MOUSE_BUTTON_DOWN: {
                    ...
                    if (event.button.button == SDL_BUTTON_LEFT) {
                        ...
                        dragging_marker = openride_app_render_marker_at_screen(&camera,
                                                           &selection,
                                                           mouse_down_x,
                                                           mouse_down_y,
                                                           width,
                ...
                case SDL_EVENT_FINGER_DOWN: {
                    ...
                    if (place_search_active) {
                        const int result = openride_app_ui_place_search_result_at(renderer,
                                                                 x,
                                                                 y,
                                                                 width,
                                                                 height,
                    ...
                    if (app_panel == OPENRIDE_APP_PANEL_MAIN) {
                        if (openride_app_ui_panel_main_search_at(renderer, x, y, width, height)) {
                            ...
                            const OpenRideAppPanel selected_panel = openride_app_ui_panel_main_at(renderer, x, y, width, height);
                    ...
                    if (app_panel == OPENRIDE_APP_PANEL_FAVORITES
                        ...
                        const uint32_t count = favorites_panel ? favorite_count : history_count;
                        OpenRideStoredPlace *items = favorites_panel ? favorite_places : history_places;
                        const int chosen_index = openride_app_ui_panel_place_at(renderer, x, y, width, height, count);
                        if (chosen_index >= 0) {
                    ...
                    if (app_panel == OPENRIDE_APP_PANEL_REGIONS) {
                        const int region_action = openride_app_ui_panel_region_action_at(renderer, x, y, width, height);
                    ...
                    dragging_marker =
                        (route_map_pick_marker != OPENRIDE_MARKER_NONE
                         || drive_mode.active)
                            ? OPENRIDE_MARKER_NONE
                            : openride_app_render_marker_at_screen(&camera,
                                           &selection,
                                           x,
                                           y,
                                           width,
                ...
                case SDL_EVENT_PINCH_UPDATE: {
                    ...
                    const double target_zoom = openride_app_support_clampd(camera.zoom + zoom_delta,
                                                       min_zoom,
                                                       max_zoom);
