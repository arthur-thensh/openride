...
static bool write_results(const OpenRideMapZoomTest *test)
{
    ...
    double frame_sum_ms=0.0,max_frame_ms=0.0,max_update_ms=0.0,max_map_ms=0.0,max_world_ms=0.0,max_masks_ms=0.0,max_areas_layer_ms=0.0,max_waterways_ms=0.0,max_roads_layer_ms=0.0,max_labels_ms=0.0,max_ui_ms=0.0,max_present_ms=0.0,max_unaccounted_ms=0.0,max_road_ms=0.0,max_road_load_ms=0.0,max_road_geometry_ms=0.0,max_road_compositor_ms=0.0,max_area_ms=0.0,max_area_load_ms=0.0,max_mask_compile_ms=0.0;
    size_t max_frame_index=0U;
    for(size_t i=0U;i<test->sample_count;++i){const OpenRideMapZoomTestSample*s=&test->samples[i];const double w=world_total_ms(&s->profile),u=sample_unaccounted_ms(s);frame_times[i]=s->frame_ms;frame_sum_ms+=s->frame_ms;if(s->frame_ms>max_frame_ms){max_frame_ms=s->frame_ms;max_frame_index=i;}if(s->profile.update_ms>max_update_ms)max_update_ms=s->profile.update_ms;if(s->profile.map_ms>max_map_ms)max_map_ms=s->profile.map_ms;if(w>max_world_ms)max_world_ms=w;if(s->profile.masks_ms>max_masks_ms)max_masks_ms=s->profile.masks_ms;if(s->profile.areas_layer_ms>max_areas_layer_ms)max_areas_layer_ms=s->profile.areas_layer_ms;if(s->profile.waterways_ms>max_waterways_ms)max_waterways_ms=s->profile.waterways_ms;if(s->profile.roads_layer_ms>max_roads_layer_ms)max_roads_layer_ms=s->profile.roads_layer_ms;if(s->profile.labels_ms>max_labels_ms)max_labels_ms=s->profile.labels_ms;if(s->profile.ui_ms>max_ui_ms)max_ui_ms=s->profile.ui_ms;if(s->profile.present_ms>max_present_ms)max_present_ms=s->profile.present_ms;if(u>max_unaccounted_ms)max_unaccounted_ms=u;if(s->road.roads_ms>max_road_ms)max_road_ms=s->road.roads_ms;if(s->road.load_ms>max_road_load_ms)max_road_load_ms=s->road.load_ms;if(s->road.geometry_ms>max_road_geometry_ms)max_road_geometry_ms=s->road.geometry_ms;if(s->road.compositor_ms>max_road_compositor_ms)max_road_compositor_ms=s->road.compositor_ms;if(s->area.areas_ms>max_area_ms)max_area_ms=s->area.areas_ms;if(s->area.load_ms>max_area_load_ms)max_area_load_ms=s->area.load_ms;if(s->area.mask_compile_ms>max_mask_compile_ms)max_mask_compile_ms=s->area.mask_compile_ms;}
    ...
    fprintf(file,"# update_ms_max=%.3f\n# map_ms_max=%.3f\n# world_ms_max=%.3f\n# masks_ms_max=%.3f\n# areas_layer_ms_max=%.3f\n# waterways_ms_max=%.3f\n# roads_layer_ms_max=%.3f\n# labels_ms_max=%.3f\n# ui_ms_max=%.3f\n# present_ms_max=%.3f\n# unaccounted_ms_max=%.3f\n# road_ms_max=%.3f\n# road_load_ms_max=%.3f\n# road_geometry_ms_max=%.3f\n# road_compositor_ms_max=%.3f\n# area_ms_max=%.3f\n# area_load_ms_max=%.3f\n# mask_compile_ms_max=%.3f\n",max_update_ms,max_map_ms,max_world_ms,max_masks_ms,max_areas_layer_ms,max_waterways_ms,max_roads_layer_ms,max_labels_ms,max_ui_ms,max_present_ms,max_unaccounted_ms,max_road_ms,max_road_load_ms,max_road_geometry_ms,max_road_compositor_ms,max_area_ms,max_area_load_ms,max_mask_compile_ms);
    fprintf(file,"frame,elapsed_ms,zoom,direction,frame_ms,update_ms,map_ms,world_ms,world_overview_ms,world_detail_ms,masks_ms,areas_layer_ms,waterways_ms,roads_layer_ms,labels_ms,ui_ms,present_ms,unaccounted_ms,visible_detail_regions,ormap_stats_valid,road_ms,road_load_ms,road_geometry_ms,road_compositor_ms,road_hits,road_misses,road_prewarm_loads,road_draw_loads,road_deferred_loads,road_prewarm_zoom,road_tiles,road_segments,road_batches,area_ms,area_load_ms,area_tiles,area_triangles,area_batches,area_prewarm_loads,area_draw_loads,area_deferred_loads,mask_tiles,mask_rects,mask_batches,mask_compile_ms,mask_compile_rects,mask_cache_hits,mask_cache_misses,mask_compile_failures,surface_plan_alpha,surface_draw_alpha,surface_plan_tiles,surface_plan_requests,surface_plan_pending,surface_plan_blending,surface_cache_entries,surface_texture_entries,surface_draw_tiles,surface_missing_data,surface_missing_textures,surface_draw_failures,surface_empty_plans\n");
    for(size_t i=0U;i<test->sample_count;++i){const OpenRideMapZoomTestSample*s=&test->samples[i];fprintf(file,"%zu,%.3f,%.5f,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%u,%u,%.3f,%.3f,%.3f,%.3f,%u,%u,%u,%u,%u,%d,%u,%u,%u,%.3f,%.3f,%u,%u,%u,%u,%u,%u,%u,%u,%u,%.3f,%u,%u,%u,%u,%.3f,%.3f,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",i,s->elapsed_ms,s->zoom,s->direction,s->frame_ms,s->profile.update_ms,s->profile.map_ms,world_total_ms(&s->profile),s->profile.world_overview_ms,s->profile.world_detail_ms,s->profile.masks_ms,s->profile.areas_layer_ms,s->profile.waterways_ms,s->profile.roads_layer_ms,s->profile.labels_ms,s->profile.ui_ms,s->profile.present_ms,sample_unaccounted_ms(s),(unsigned)s->profile.visible_detail_regions,s->profile.ormap_stats_valid?1U:0U,s->road.roads_ms,s->road.load_ms,s->road.geometry_ms,s->road.compositor_ms,(unsigned)s->road.cache_hits,(unsigned)s->road.cache_misses,(unsigned)s->road.prewarm_loads,(unsigned)s->road.draw_loads,(unsigned)s->road.deferred_loads,s->road.prewarm_zoom,(unsigned)s->road.tiles_visited,(unsigned)s->road.segments_drawn,(unsigned)s->road.batches,s->area.areas_ms,s->area.load_ms,(unsigned)s->area.tiles_visited,(unsigned)s->area.triangles_drawn,(unsigned)s->area.batches,(unsigned)s->area.prewarm_loads,(unsigned)s->area.draw_loads,(unsigned)s->area.deferred_loads,(unsigned)s->area.mask_tiles,(unsigned)s->area.mask_rects,(unsigned)s->area.mask_batches,s->area.mask_compile_ms,(unsigned)s->area.mask_compile_rects,(unsigned)s->area.mask_cache_hits,(unsigned)s->area.mask_cache_misses,(unsigned)s->area.mask_compile_failures,s->area.surface_plan_alpha,s->area.surface_draw_alpha,(unsigned)s->area.surface_plan_tiles,(unsigned)s->area.surface_plan_requests,(unsigned)s->area.surface_plan_pending,(unsigned)s->area.surface_plan_blending,(unsigned)s->area.surface_cache_entries,(unsigned)s->area.surface_texture_entries,(unsigned)s->area.surface_draw_tiles,(unsigned)s->area.surface_missing_data,(unsigned)s->area.surface_missing_textures,(unsigned)s->area.surface_draw_failures,(unsigned)s->area.surface_empty_plans);}
